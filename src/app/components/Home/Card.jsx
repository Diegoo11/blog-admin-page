  ...
  return (
    <>
      <article className="flex flex-col py-8">
        <div className="flex flex-col sm:flex-row gap-x-2 gap-y-3 justify-between items-center">
          <div className="flex flex-col gap-4 w-full max-w-4xl">
            ...
            <footer className="hidden md:flex justify-between items-center mt-5 text-sm">
              <div className="flex gap-4 justify-center items-center">
                <Link href={`${URL}/${type}`} className="px-2 py-1 bg-gray-200 rounded-full">
                  {type}
                </Link>
                <Link href={`${URL}/${type}/${path}`}>
              ...
              <div className="flex justify-center items-center gap-2">
                <Link href={`${URL}/${type}/${path}`} aria-label="compartir" className="p-1 text-[#94979b] hover:text-[#4e4f52]">
                ...
                <div className="flex ml-5 gap-3 justify-center items-center">
                  <Link href={`/admin/editar?path=${path}`} className="text-[#6f7175] hover:text-black text-base">
                    Editar
                  </Link>
                  <DeleteButton
                    title={title}
        ...
        <footer className="flex md:hidden justify-between items-center mt-5 text-sm flex-wrap gap-x-2 gap-y-2">
          <div className="flex gap-4 justify-start items-center flex-wrap">
            ...
            <Link href={`${URL}/${type}/${path}`} className="ml-2">
              {Math.ceil(content.length / 550)}
              {' '}
              min
            </Link>
